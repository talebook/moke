...
/**
 * A successful guest response confirms logout. Transport, HTTP, and unexpected
 * application failures throw so callers can retain their last confirmed user.
 */
export async function readCurrentUserResponse(response: Response): Promise<CurrentUserResult> {
  let data: UserInfoResponse;

  try {
    data = await readJsonResponse<UserInfoResponse>(response, '登录状态响应无效。');
  } catch (error) {
    if (error instanceof MokeApiError && response.status === 401) {
      return {
        err: 'user.need_login',
        msg: error.message,
        user: null,
  ...
  const isConfirmedGuest = response.status === 401
    || data.err === 'user.need_login'
...
/** A rejected sync is unknown, not a confirmed logout. */
